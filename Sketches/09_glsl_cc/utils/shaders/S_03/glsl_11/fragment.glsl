...
float polygon(vec2 pt,vec2 center,float radius,int sides,float rotate,float edge_thickness){
  ...
  float d=cos(floor(.5+theta/rad)*rad-theta)*length(pt);
  return 1.-smoothstep(radius,radius+edge_thickness,d);
}
...
void main(){
  ...
  color+=polygon(v_uv,vec2(.9-sin(time*3.)*.05,.5),.005,3,0.,.001)*vec3(1.);
  color+=polygon(v_uv,vec2(.1-sin(time*3.+PI)*.05,.5),.005,3,PI,.001)*vec3(1.);
